...
public class CustomerController {
	...
	public ResponseEntity<List<Customer>> loginValidate(@PathVariable String keyword) {
		List<Customer> listCustomer= customerRepository.findByNameOrEmailOrAddressOrNicOrBirthdayOrMobileOrGenderOrCustomerType(keyword);
		if (!listCustomer.isEmpty()) {
			return ResponseEntity.ok(listCustomer);
		} else {
			List<Customer> listCustomerEmpty = List.of(new Customer());
			return ResponseEntity.ok(listCustomerEmpty);
		}
